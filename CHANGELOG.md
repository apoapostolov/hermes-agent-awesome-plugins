...
- **rss-reader:** `regrade_all` Hermes tool action (both editions). Re-runs grading over every article in the library, already-graded ones included, so rubric changes apply retroactively without a manual reset. The desktop command loops grading passes until the library is exhausted (12-pass cap at 60 per pass).
