...
def _break_oldest(*, hint: Optional[str], again: bool, tool_call_id: Optional[str] = None) -> str:
    with _lock:
        ...
        # Public edition does not SIGKILL descendant processes. Hooks relabel
        # the in-flight call. Personal edition keeps the process-kill path.
    waited = _fmt_duration((time.monotonic() - mark["started_at"]) * 1000)
    extra = ""
    if again:
...
def register(ctx: Any) -> None:
    ...
    # Public edition: VALID_HOOKS only. No process-wide Popen patch and no
    # private HermesCLI method rebind (catalog rule 8 / request-changes).
